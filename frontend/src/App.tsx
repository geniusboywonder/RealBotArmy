...
export default function App() {
  ...
  // Add role interaction matrix state
  const [roleInteractions, setRoleInteractions] = useState(() => {
    // Create matrix for all agents, not just unique roles
    const agentRoles = agents.map(agent => `${agent.role} ${agent.id}`);
    const matrix = {};
    agentRoles.forEach(role1 => {
      matrix[role1] = {};
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse table-fixed">
                      <thead>
                        <tr>
                          <th className="w-40 p-4 border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 font-medium text-left">
                            Agent
                          </th>
                      ...
                      <tbody>
                        {Object.keys(roleInteractions).map(role1 => (
                          <tr key={role1} className="h-20">
                            <td className="w-40 p-4 border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 font-medium text-left align-middle text-sm">
                              {role1}
                            </td>
